...
class TranscriptCliTestCase(unittest.TestCase):
    ...
    def run_single(self, fixture_name: str, *extra_args: str, expected_code: int = 0):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Decode JSON-embedded turns so content assertions work transparently
            m = re.search(r"window\.__TURNS\s*=\s*(\[.*?\])\s*;", html, re.DOTALL)
            if m:
                turns = json.loads(m.group(1))
                html = html[: m.start()] + "\n".join(turns) + html[m.end() :]
            return result, html
...
class ClaudeTranscriptCliTests(TranscriptCliTestCase):
    ...
    def test_redact_flag_redacts_sensitive_values(self):
        result, html = self.run_single("share_sample.jsonl", "--redact")
    ...
    def test_hide_timestamps_and_tool_results(self):
        _, html = self.run_single("share_sample.jsonl", "--hide", "timestamps", "tool-results")
        self.assertNotIn("10:00:00", html)
        self.assertNotIn("2026-03-01", html)

    def test_hide_cost(self):
        _, html = self.run_single("share_sample.jsonl", "--hide", "cost")
    ...
    def test_hide_diffs(self):
        _, html = self.run_single("flags_sample.jsonl", "--hide", "diffs")
        self.assertNotIn('class="diff-block"', html)

    def test_hide_icons(self):
        _, html = self.run_single("flags_sample.jsonl", "--hide", "icons")
        self.assertNotIn('class="turn-icon"', html)

    def test_hide_thinking(self):
        _, html = self.run_single("flags_sample.jsonl", "--hide", "thinking")
        self.assertNotIn('class="thinking-block"', html)

    def test_hide_compactions(self):
        _, html = self.run_single("flags_sample.jsonl", "--hide", "compactions")
    ...
    def test_back_to_back_compactions_collapsed(self):
        """3 consecutive compactions should render as 1 (the last one)."""
        _, html = self.run_single("back_to_back_compaction.jsonl")
        comp_count = html.count('class="compaction"')
        self.assertEqual(comp_count, 1, f"Expected 1 compaction, got {comp_count}")
        # Verify it kept the last one (30000 tokens)
        self.assertIn("30000", html)

    def test_compaction_no_tokens_no_empty_parens(self):
        """Compaction with no token count should not show '( tokens)'."""
        _, html = self.run_single("compaction_no_tokens.jsonl")
        self.assertIn('class="compaction"', html)
    ...
    # -- Demo fixture tests (complex, feature-rich conversations) --

    def test_demo_perf_has_thinking_and_tools(self):
        _, html = self.run_single("mock-perf.jsonl")
        self.assertIn('class="thinking-block"', html)
        self.assertIn("Seq Scan on articles", html)
        self.assertIn("183x faster", html)

    def test_demo_debug_renders_error_output(self):
        _, html = self.run_single("mock-debug.jsonl")
        self.assertIn("badge", html)

    def test_demo_refactor_has_diff(self):
        _, html = self.run_single("mock-refactor.jsonl")
        self.assertIn("diff-", html)

    def test_demo_devops_has_tool_calls(self):
        _, html = self.run_single("mock-devops.jsonl")
        self.assertIn("Dockerfile", html)
        self.assertIn("docker-compose", html)

    def test_demo_session_renders(self):
        _, html = self.run_single("mock-session.jsonl")
...
class ExtractHtmlMetaTests(unittest.TestCase):
    ...
    def test_extracts_date_and_date_raw(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(meta["date_raw"], "2026-03-07 06:00:00")
...
class CodexTranscriptCliTests(TranscriptCliTestCase):
    ...
    def test_redact_flag_redacts_sensitive_values(self):
        result, html = self.run_single("codex_share_sample.jsonl", "--redact")
        self.assertIn("[REDACTED_EMAIL]", html)
        self.assertIn("[REDACTED_IP]", html)
        self.assertIn("[REDACTED_SECRET]", html)
        self.assertIn("/Users/REDACTED/project", html)
    ...
    def test_hide_timestamps_and_tool_results(self):
        _, html = self.run_single("codex_share_sample.jsonl", "--hide", "timestamps", "tool-results")
...
def run_claude_integration_suite() -> bool:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if rc != 0:
            ...
            checks = [
                ("hide-thinking", ["--hide", "thinking"], lambda html: 'class="thinking-block"' not in html),
                ("hide-tools", ["--hide", "tools"], lambda html: 'class="tools-section"' not in html),
                ("hide-diffs", ["--hide", "diffs"], lambda html: 'class="diff-block"' not in html),
                ("hide-icons", ["--hide", "icons"], lambda html: 'class="turn-icon"' not in html),
                ("title", ["--title", "Integration Title"], lambda html: "<title>Integration Title</title>" in html),
                ("redact", ["--redact"], lambda html: "fonts.googleapis.com" in html),
            ]
