...
def build_base_parser(description: str, default_outdir: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="SECTION",
        help="Hide sections: thinking, tools, tool-results, diffs, icons, compactions, cost, timestamps, toc",
    )
    ...
    parser.add_argument("--redact", action="store_true", help="Redact PII (emails, IPs, paths, API keys, env vars)")
    parser.add_argument("--redact-home", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument("--redact-env", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument("--redact-email", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument("--redact-ip", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument("--redact-api-keys", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument("--redact-paths", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument(
        "--redact-pattern",
        action="append",
        default=[],
...
def apply_hide_flags(args: argparse.Namespace) -> None:
    """Expand --hide and --redact into internal flags."""
    # Parse --hide: accept comma-separated and/or space-separated values
    hide_set: set[str] = set()
    for item in getattr(args, "hide", []):
        for part in item.split(","):
            p = part.strip()
            if p:
                hide_set.add(p)
    for key, attr in HIDE_OPTIONS.items():
        if key in hide_set:
            setattr(args, attr, True)
        elif not getattr(args, attr, False):
...
def serialize_forwarded_flags(args: argparse.Namespace) -> list[str]:
    ...
    # Forward --hide sections
    hide_parts = [key for key, attr in HIDE_OPTIONS.items() if getattr(args, attr, False)]
    if hide_parts:
        extra_flags.append("--hide")
        extra_flags.extend(hide_parts)
...
def build_html_scaffold_prefix(
    ...
    return [
        ...
        f".skeleton-inner {{ max-width: {max_width}; margin: 0 auto; padding: 20px 32px; }}",
        ...
        # Pagination
        ".pagination { text-align: center; padding: 16px 0; font-family: 'Inter',-apple-system,'Segoe UI','Helvetica Neue',Arial,sans-serif; }",
        ".pg-btn { padding: 6px 16px; border: 1px solid #ddd; border-radius: 4px; background: #fff; cursor: pointer; font-size: 13px; font-family: inherit; }",
        ".pg-btn:disabled { opacity: 0.4; cursor: default; }",
        ".pg-btn:hover:not(:disabled) { background: #f5f5f5; }",
        ".pg-info { margin: 0 16px; color: #888; font-size: 13px; }",
        ...
        " if(window.__pgAll)window.__pgAll();"
        ...
        "   if(window.__pgRestore)window.__pgRestore();"
        "   return;"
        " }"
        ...
        + '</div></div>'
        '</div></div>',
        '<div class="skeleton"><div class="skeleton-inner">'
        ...
        + '</div></div>'
        '<script>window.addEventListener("load",function(){document.body.classList.add("loaded");var p=document.querySelector(".page");if(p)p.style.display=""});</script>',
        '<div class="page" style="display:none"><div class="main">',
...
JS_PAGINATION = (
    "(function(){"
    "var T=window.__TURNS;if(!T||!T.length)return;"
    "var C=document.getElementById('turns-container');if(!C)return;"
    "var PS=50,CP=0,TP=Math.ceil(T.length/PS);"
    "var pag=document.createElement('div');pag.className='pagination';C.after(pag);"
    "function rp(p){"
    "CP=p;C.innerHTML=T.slice(p*PS,Math.min((p+1)*PS,T.length)).join('\\n');"
    "if(TP<=1){pag.style.display='none';return;}"
    "pag.style.display='';"
    "pag.innerHTML="
    "'<button class=\"pg-btn\" onclick=\"window.__pgPrev()\"'+(p===0?' disabled':'')+'>&larr; Prev</button>'"
    "+'<span class=\"pg-info\">Page '+(p+1)+' / '+TP+'</span>'"
    "+'<button class=\"pg-btn\" onclick=\"window.__pgNext()\"'+(p>=TP-1?' disabled':'')+'>&rarr; Next</button>';"
    "}"
    "window.__pgPrev=function(){if(CP>0){rp(CP-1);C.scrollIntoView({block:'start'});}};"
    "window.__pgNext=function(){if(CP<TP-1){rp(CP+1);C.scrollIntoView({block:'start'});}};"
    "window.__pgGo=function(p){rp(Math.max(0,Math.min(p,TP-1)));};"
    "window.__pgAll=function(){C.innerHTML=T.join('\\n');pag.style.display='none';};"
    "window.__pgRestore=function(){rp(CP);};"
    "window.__pgFind=function(n){"
    "for(var i=0;i<T.length;i++){"
    "if(T[i].indexOf('id=\"turn-'+n+'\"')!==-1){"
    "var p=Math.floor(i/PS);if(p!==CP)rp(p);"
    "setTimeout(function(){var el=document.getElementById('turn-'+n);"
    "if(el){el.classList.remove('collapsed');el.scrollIntoView({block:'start'});}},50);"
    "return;}}};"
    "function hh(){var m=location.hash.match(/turn-(\\d+)/);if(m)window.__pgFind(parseInt(m[1]));}"
    "rp(0);hh();window.addEventListener('hashchange',hh);"
    "})();"
)
...
def render_html(
    ...
    *,
    ...
    prev_ts = None
    turn_num = 0
    scaffold_end = len(out)
    turn_block_starts: list[int] = []
    for turn in turns:
        turn_block_starts.append(len(out))
        ...
        if turn["type"] == "compaction":
            if args.no_compactions:
                continue
            # Skip consecutive compactions — keep only the last one
            # Check all lines since previous block start (may include time-gap)
            prev_start = turn_block_starts[-2] if len(turn_block_starts) >= 2 else scaffold_end
            prev_block = out[prev_start:]
            if any('class="compaction"' in line for line in prev_block):
                # Remove the entire previous compaction block (gap + compaction)
                del out[prev_start:]
                turn_block_starts.pop()  # remove current start
                turn_block_starts[-1] = len(out)  # reset to current position
            for index in range(len(out) - 1, -1, -1):
                if '<div class="turn ' in out[index]:
                    out[index] = out[index].replace('<div class="turn ', '<div class="turn before-gap ', 1)
                    break
            ...
            if not args.no_timestamps:
                if tok:
                    label += f" ({tok} tokens) - {ts_fmt(turn['ts'])}"
                else:
                    label += f" - {ts_fmt(turn['ts'])}"
            elif tok:
                label += f" ({tok} tokens)"
    ...
    # Extract turn blocks from out, embed as JSON for JS pagination
    turn_blocks: list[str] = []
    for i in range(len(turn_block_starts)):
        start = turn_block_starts[i]
        end = turn_block_starts[i + 1] if i + 1 < len(turn_block_starts) else len(out)
        if end > start:
            block_html = "\n".join(part for part in out[start:end] if part)
            if block_html.strip():
                turn_blocks.append(block_html)
    del out[scaffold_end:]

    out.append('<div id="turns-container"></div>')
    turns_json = json.dumps(turn_blocks).replace("</", "<\\/")
    out.append(f"<script>window.__TURNS={turns_json};</script>")
    out.append(f"<script>{JS_PAGINATION}</script>")
...
def extract_html_meta(path: str) -> dict[str, str]:
    ...
    if date_m:
        ...
        if ts_m:
            ...
            # Sort by last message timestamp (end of range), fall back to start
            end_m = re.search(r"-\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})", raw)
            date_raw = end_m.group(1).strip() if end_m else raw[:19]
    # Extract turns count
    turns = ""
